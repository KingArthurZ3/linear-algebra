...
class Plane(object):
    ...
    def __eq__(self, p):
        ...
        if self.is_parallel_to(p):
            n1 = self.basepoint.minus(p.basepoint)
            if n1.is_orthogonal_to(self.normal_vector):
                return True
        return False
...
# print("p1 and p2: (equal?)", p1 == p2, " (parallel?)", p1.is_parallel_to(p2))
#
